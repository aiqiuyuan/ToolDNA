...
def compute_score(data_source: str, solution_str: str, ground_truth: str, extra_info: Dict = None) -> float:
    ...
    DEBUG_LOG_PATH = "/mnt/bn/motor-nlp-team/users/aiqiuyuan/verl_debug/debug_records_0623_dynamic_2.jsonl"
    SCORE_LOG_PATH = "/mnt/bn/motor-nlp-team/users/aiqiuyuan/verl_debug/score_records_0623_dynamic_2.jsonl"
    ...
    solution_actions = parse_actions(extract_action_content(solution_str))
    gt_actions = parse_actions(extract_action_content(ground_truth))
    ...
    total = format_score -1 + tool_score * 0.4 + tool_name_score * 0.2 + param_name_score * 0.2 + param_value_score * 0.2
    #print(f"[REWARD SCORE] Format: {format_score:.2f}, Tool: {tool_score:.2f}, Description: {has_description:.2f}, Total: {total:.2f}")
    print(f"[REWARD SCORE] Format: {format_score:.2f}, Tool Name: {tool_name_score:.2f}, Param Name: {param_name_score:.2f}, Param Value: {param_value_score:.2f}, Tool Call: {tool_score:.2f},  Description: {has_description:.2f}, Total: {total:.2f}")
