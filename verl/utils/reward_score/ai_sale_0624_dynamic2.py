...
def parse_actions(actions_text: str) -> List[Dict]:
    ...
    # 匹配格式: [工具1(参数1),工具2(参数2),...]
    tool_pattern = r'\[([^][]*)\]'
    tool_match = re.search(tool_pattern, clean_text)
    
    parsed_actions = []
    tool_format_score = 0
    
    if tool_match:
        tool_list_str = tool_match.group(1)
        all_params_valid = True
        
        # 优化的工具调用匹配模式，支持复杂嵌套和格式容错
        action_pattern = r'([a-zA-Z_]+)\s*\(\s*({.*?})\s*\)'
        action_matches = re.finditer(action_pattern, tool_list_str, re.DOTALL)
        
        for match in action_matches:
            tool_name = match.group(1).strip()
            param_str = match.group(2).strip()
            
            # 尝试修复参数格式（处理单引号和不完整括号）
            param_str = re.sub(r"'([^']+)'", r'"\1"', param_str)  # 单引号转双引号
            param_str = param_str.rstrip('),')                     # 移除末尾多余符号
...
def compute_score(data_source: str, solution_str: str, ground_truth: str, extra_info: Dict = None) -> float:
    ...
    DEBUG_LOG_PATH = "/mnt/bn/motor-nlp-team/users/aiqiuyuan/verl_debug/debug_records_0629_dynamic_2_16.jsonl"
    SCORE_LOG_PATH = "/mnt/bn/motor-nlp-team/users/aiqiuyuan/verl_debug/score_records_0629_dynamic_2_16.jsonl"
...
if __name__ == "__main__":
    ...
    <actions>\n[call_human({\"reason\": \"视频看车\"}), check_policy({\"registration_city\": \"杭州\"}), calculator({\"expression\": \"30000/129900\"}), calculate_loan({\"total_price\": 12.99, \"down_payment_ratio\": 0.23, \"periods\": 36}), calculate_loan({\"total_price\": 12.99, \"down_payment_ratio\": 0.23, \"periods\": 48}), calculate_loan({\"total_price\": 12.99, \"down_payment_ratio\": 0.23, \"periods\": 60}), calculator({\"expression\": \"(129900*0.23+1000+2500)/36\"}, \"expression\": \"(129900*0.23+1000+2500)/48\"}, \"expression\": \"(129900*0.23+1000+2500)/60\")]\n</actions>
    """
    test_ground_truth1 = """
    <thought>
    1. 用户回复“发给我看看”...
    ...
    ##测试1:<actions>\n[search_used_cars({\"query\": \"奔驰C260\", \"filters\": {\"price_range\": [20, 21], \"source_city\": \"深圳\"}}), search_used_cars({\"query\": \"奔驰C200\", \"filters\": {\"price_range\": [20, 21], \"source_city\": \"深圳\"}}), search_used_cars({\"query\": \"宝马X5\", \"filters\": {\"price_range\": [20, 21], \"source_city\": \"深圳\"}})]\n</actions>
    ##测试2:<actions>\n[check_delivery_fee({\"source_city\":\"武汉\",\"registration_city\":\"宝鸡\"}), check_policy({\"registration_city\":\"宝鸡\"})]\n</actions>
